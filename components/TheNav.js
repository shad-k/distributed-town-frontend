      ...
      {links && links.length > 0 && (
        <>
            ...
            {links.map((link, index) => {
              return (
                <a
                  key={index}
                  href={link.url}
                  className="uppercase block mt-4 lg:inline-block lg:mt-0 text-gray-600 hover:text-gray-900  mr-4"
                >
                  {link.text}
                </a>
              );
          ...
          <div>
            <a
              href={helpUrl}
              className="inline-block text-sm px-4 py-2 leading-none border bg-gray-500 rounded text-white hover:border-transparent hover:text-white-500 hover:bg-gray-600 mt-4 lg:mt-0"
            >
              {helpCta}
            </a>
          </div>
